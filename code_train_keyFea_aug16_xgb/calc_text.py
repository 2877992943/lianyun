...
def gen_wordList(wordlist):
    ...
    for word in wordlist:
        ...
        if len(word)>=4:
            parseStr=wordParsing(word)
            rstlist+=parseStr.split(' ')
        else:
            rstlist.append(word)
    return rstlist
...
def calculate_text(raw_str): #return wordlist
    ...
    try:
        ...
        words = re.split(split_pattern, raw_str);print '11',words
        words = filter(lambda w: w and len(w) > 0, words);print '12',words
        n_gram_wordList=[word.decode('utf-8') for word in words];print n_gram_wordList# 数据服务
        one_gram_wordList=gen_wordList(n_gram_wordList);print one_gram_wordList # 数据  服务
        wlist=set(n_gram_wordList+one_gram_wordList);print wlist
