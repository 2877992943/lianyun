...
if __name__ == "__main__":
    ...
    attList='Clue_Entry_Com_Name_clean,Clue_Entry_Name_clean,Clue_Entry_Cellphone_clean,Com_Address_clean,Clue_Entry_Major_clean,legal_person_clean,main_produce_clean,CLue_Id'.split(',')


    ### batch query sql  clean & unclean

    # df=get_clue_idx('clean');print df.shape
    ...
    for i in range(num+1):
        ...
        ret=query_by_att(attList,cids)
        pd.to_pickle(ret,'../clean_atts/%s'%cids[0])
    """

    # ######
    # df=get_clue_idx('unclean');print df.shape
    # df.to_csv("../data/clueIds.csv",index=False,encoding='utf-8')
    #
    # df=pd.read_csv('../data/clueIds.csv',encoding='utf-8')
    # cidList=df['CLue_Id'].values.tolist()
