...
class mySentence(object):
    ...
    def __iter__(self):
        ...
        df=pd.read_csv(self.path,encoding='utf-8');print df.shape
        all_clueId=df['CLue_Id'].values.tolist()
        numall=len(all_clueId)
        batch_sz=100000
        num_batches=numall/batch_sz+1
        for i in range(num_batches)[i_now:]:
...
if __name__ == "__main__":
    ...
    gene=mySentence('../data/cid.csv')
     
    i_now=18
    for model_clueid in gene: # 0 1 2 3
        print 'batch',i_now
        
         
        #model_clueid=all_clueId[:2]

        fct(model_clueid,i_now)
        i_now+=1
