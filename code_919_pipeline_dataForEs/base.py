...
@dict_result_cache(CLUE_CACHE)#dict_result_cache(func,dictCache)  use get_clues() as parameters execute inner() decorator()
def get_clues(clue_ids):
    ...
    end_time=time.time();#print 'ret CLUECACHE',len(ret),len(CLUE_CACHE),ret.values()[0],ret.values()[0]['CLue_Id'],len(ret),ret.keys()[0]
    #pd.to_pickle(CLUE_CACHE,'../cache_batches/rst_c_%s'% str(ret.keys()[0]))
    #pd.to_pickle(ret,'../cache_batches/rst_%s'% str(ret.keys()[0]))
    print 'len cache',len(CLUE_CACHE),len(FEATURE_VALUE_CACHE)   # here CLUE_CACHE.clear()???
...
class CategoricalTextMixIn(object):
    ...
    def get_raw_str(cls, object_id):
        ...
        dic = cls.get_data(object_id);#print 'get raw str',dic,dic[cls.field_name] all attribute
    ...
    def calculate(cls, object_id): #words->wordlist->widlist
        ...
        if raw_str is None:
            return []
        #words = re.split(cls.split_pattern, raw_str)# only split by ,/|# ,not segmentWord  string->list
        words=calculate_text(raw_str)
        words = filter(lambda w: w and len(w) > 0, words)
        #print 'calc'
        #for w in words:
