...
if __name__ == "__main__":
    ...
    for i in range(num_batches)[8:]: # 0 1 2 3
        ...
        # combine fid & raw
        raw_fid_dict=combine_raw_fid(cid_fid_dict,CLUE_CACHE)# {cid:{rawdict...}...}
         
        # select field |time to timestamp | lagitude longitude ->location |businessScope
        actions=process_data(raw_fid_dict)
        ...
        # clear()
        # time.sleep(5)
        print "CLUE_CACHE length", len(CLUE_CACHE),len(FEATURE_VALUE_CACHE)
        CLUE_CACHE.clear()
        FEATURE_VALUE_CACHE.clear()
        print "CLUE_CACHE length", len(CLUE_CACHE),len(FEATURE_VALUE_CACHE)
