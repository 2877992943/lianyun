...
def filter_idf(word_idf_dict,ll,topn):
    wordi_idf={}
    for word in ll:
    ...
    n=topn if len(ss)>=topn else len(ss)
    wl=[w[0] for w in ss][:n]
    idf=[w[1] for w in ss][:n]

    return [wl,idf]



def query_comname(comname):
    sql ="""
    SELECT company_name,comname_keyword
    from company_name_keyword
    ...
    cur = db.cursor()
    #print sql%comname
    cur.execute(sql%comname)
    ret = {}
    for r in cur.fetchall():
        ret[r['company_name']] = r['comname_keyword'] #{id:{record},...}
    return ret
...
if __name__ == "__main__":
    ...
    for it in gene:
        ...
        ### comname
        print 'comname',cid#,parse
        #ret=query_comname(cid.decode('utf-8'));print ret.values()[0]
        #comname_k_vec=[word2vec(word) for word in ret.values()[0].split(' ') if word2vec(word)!=''];print len(comname_k_vec)
        #comnamell=ret.values()[0].split(' ')
        ...
        centers=kmeans.cluster_centers_;print 'centrio', centers.shape
        ####
        cluster_idfMean_dict={}
        for topic in range(n_topic)[:]:
            ...
            striList_unique,idf_k=filter_idf(word_idf_dict,word_list,20)
            cluster_idfMean_dict[' '.join(striList_unique)]=np.mean(idf_k)
            #print ' '.join(striList_unique)
            #print 'this cluster idf',np.mean(idf_k[:int(len(idf_k)/2.)]),np.mean(idf_k[:]),np.std(idf_k[:]),np.min(idf_k[:]),np.max(idf_k[:])
        ss=sorted(cluster_idfMean_dict.iteritems(),key=lambda s:s[1],reverse=True)
        ss=[s[0]for s in ss[:2]]
        cominfo1,cominfo2=ss# cominfo1 has priority over cominfo2 because of idf
        # if np.sum([cominfo1.count(comname) for comname in comnamell])<=np.sum([cominfo2.count(comname) for comname in comnamell]):
        #     print cominfo1
        # else:print cominfo2
        print cominfo1
        print cominfo2
